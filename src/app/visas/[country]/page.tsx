...
export default function CountryVisaPage({ params }: { params: { country: string } }){
  ...
  const [travelDate, setTravelDate] = useState('2025-11-23');
  ...
  const cd: CountryData | undefined = (countryData as Record<string, CountryData>)[countrySlug]
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm mb-6">
          <div className="flex items-start justify-between gap-6">
            <div>
              <h1 className="text-2xl font-bold">{countryName} Visa <span className="text-amber-500">★★★★★</span></h1>
            ...
            <div className="hidden md:block">
              <button className="btn-brand px-4 py-2 rounded-md text-white">BOOK NOW</button>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-0" style={{overflow: 'visible'}}>
          <div className="p-6 border-b">
            <h2 className="text-lg font-semibold">{countryName} Visa Prices & Options</h2>
          </div>
          ...
          <div className="p-6 flex flex-col lg:flex-row gap-6">
            <div className="flex-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <label className="flex flex-col text-sm">
                  ...
                  <select value={processing} onChange={(e)=>setProcessing(e.target.value)} className="px-3 py-2 rounded-md border border-slate-200 bg-white text-sm focus:outline-none">
                    {processingOptions.map(opt => (<option key={opt}>{opt}</option>))}
                  </select>
                </label>
                ...
                <label className="flex flex-col text-sm">
                  ...
                  <select value={String(count)} onChange={handleCountChange} className="px-3 py-2 rounded-md border border-slate-200 bg-white text-sm focus:outline-none">
                    {[...Array(10)].map((_, i) => (
              ...
              <div className="mt-6 border-t pt-4 flex items-center justify-end gap-4">
                ...
                <button className="px-4 py-2 rounded-md bg-amber-500 text-white" onClick={() => {
            ...
            <div className="w-full lg:w-72">
              <div className="bg-slate-50 p-5 rounded-lg shadow-inner text-right">
                ...
                <div className="text-xl font-bold text-amber-500 mt-2"><Price amountUSD={basePriceUSD * count} /></div>
              </div>
            </div>
          </div>
        </div>
            ...
            {countrySlug === 'united-states' && (
              <section className="mt-10">
                <div className="bg-white rounded-xl shadow-sm p-8">
                  <h2 className="text-2xl font-bold mb-4 text-blue-900">USA Visa Online</h2>
                  {description && description.split(/\n+/).map((para, idx) => (
                    <p key={idx} className="text-slate-700 mb-4 leading-relaxed">{para}</p>
                  ))}
                  <h3 className="text-xl font-semibold mt-8 mb-3 text-blue-800">USA Visa Documents</h3>
                  <ul className="list-disc pl-6 text-slate-700 space-y-2">
                    {requiredDocuments.map((doc, idx) => (
                      <li key={idx}>{doc}</li>
                    ))}
                  </ul>
                  {visaTypeDocuments && Object.keys(visaTypeDocuments).length > 0 && (
                    <div className="mt-6">
                      <h4 className="text-lg font-semibold text-blue-700 mb-2">Depending on the visa type:</h4>
                      <ul className="list-disc pl-6 text-slate-700 space-y-1">
                        {Object.entries(visaTypeDocuments).map(([type, docs], idx) => (
                          <li key={type} className="mb-2">
                            <span className="font-medium">{type}:</span> {docs.join(', ')}
                          </li>
                        ))}
                      </ul>
                    </div>
