...
const tabs = ["Activities", "Hotels", "Holidays", "Visa", "Cruise"];
...
export default function ServicesTabs() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8">
      <div
        className="flex gap-4 overflow-x-auto overflow-y-hidden snap-x services-tabs"
        role="tablist"
        aria-label="Services"
      >
        {tabs.map((t) => {
          const content = (
            ...
            >
              {t}
              <span
                className={`block h-0.5 mt-2 rounded-full transition-all ${active === t ? "bg-white w-6" : "bg-transparent w-0"}`}
              ></span>
            </button>
          );
