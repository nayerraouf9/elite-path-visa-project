...
export default function DatePicker({ value, onChange }: Props) {
  ...
  return (
    <div className="relative" ref={rootRef} onKeyDown={onKey} style={{overflow: 'visible'}}>
      ...
      {open && (
        <div className="absolute z-[9999] mt-2 bg-white rounded-xl shadow-2xl p-4 w-80 text-sm" role="dialog" aria-label="Calendar" style={{overflow: 'visible'}}>
          <div className="flex items-center justify-between mb-3">
            <button type="button" onClick={handlePrev} className="p-2 rounded-md hover:bg-slate-100" aria-label="Previous month">‹</button>
            <div className="text-center">
              <div className="font-semibold">{viewDate.toLocaleString(undefined,{ month: 'long' })} {viewDate.getFullYear()}</div>
            </div>
            <button type="button" onClick={handleNext} className="p-2 rounded-md hover:bg-slate-100" aria-label="Next month">›</button>
          </div>
          ...
          <div className={`overflow-hidden`}>
            <div className={`grid grid-cols-7 gap-1 transition-transform duration-200`}> 
              {weeks.map((d, idx) => (
                <div key={idx} className="h-9">
                  {d ? (
                    <button type="button" onClick={() => selectDay(d)} className={`w-full h-9 rounded-md transition-colors ${d.toDateString() === selected.toDateString() ? 'bg-amber-500 text-white' : 'text-slate-700 hover:bg-slate-100'}`} aria-pressed={d.toDateString() === selected.toDateString()}>
                      {d.getDate()}
                    </button>
