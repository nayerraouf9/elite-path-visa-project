...
export default function CheckoutModal() {
  ...
  useEffect(() => {
    if (isCheckoutModalOpen) {
      ...
      else if (selectedIndex >= cartItems.length) setSelectedIndex(cartItems.length - 1);
    }
    ...
    <div className="bg-white rounded-xl shadow p-0 mt-2 flex flex-col md:flex-row w-full">
        ...
        <form className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 md:gap-4 mb-4 text-sm md:text-base" autoComplete="off">
          <div>
            <select
              ...
              value={lead.title}
              onChange={e => setLead((prev:any) => ({ ...prev, title: e.target.value }))}
              autoComplete="off"
            >
              <option>Mr.</option>
              <option>Ms.</option>
      ...
      <div className="w-full md:w-96 border-t md:border-l md:border-t-0 bg-slate-50 p-4 md:p-6 flex flex-col justify-between">
        ...
        <div className="mt-4 text-right">
          ...
          <div className="mt-4">
            <button className="px-6 py-2 rounded-md bg-amber-500 text-white font-semibold" onClick={()=>{ if(!selected) return; setStep(2); }}>
              Next
            </button>
          </div>
        </div>
    ...
    <div className="bg-white rounded-xl shadow p-0 mt-2 flex max-w-4xl mx-auto">
      ...
      <div className="w-96 border-l bg-slate-50 p-6 flex flex-col justify-between">
        <div>
          ...
          <div className="mb-2 text-sm">Total Amount <span className="float-right">AED <Price amountUSD={selected?.totalPrice || 0} /></span></div>
          <div className="bg-orange-100 rounded-lg p-4 text-center mt-4">
            <span className="font-bold text-xl">Final Amount</span>
            <span className="font-bold text-xl ml-4 text-amber-500">AED <Price amountUSD={selected?.totalPrice || 0} /></span>
          </div>
        </div>
        <div className="mt-6 text-right">
          <button type="button" className="px-6 py-2 rounded-md bg-amber-500 text-white font-semibold" onClick={()=>setStep(3)}>
            Next
          </button>
        </div>
      </div>
...
function Step3({ method, setMethod, agree, setAgree, setStep, setIsCheckoutModalOpen }: any){
  return (
    <div className="bg-white rounded-xl shadow p-0 mt-2 max-w-3xl mx-auto">
      <div className="p-6">
        <div className="flex items-center gap-2 mb-4"><span className="text-2xl text-amber-500"><i className="fa fa-credit-card" /></span><span className="font-bold text-xl">Choose a Payment Method</span></div>
        <div className="mb-6">
        ...
        <div className="flex justify-end gap-2">
          <button className="px-4 py-2 rounded-md border" onClick={()=>setStep(2)}>Back</button>
          <button type="button" className="px-6 py-2 rounded-md bg-orange-500 text-white font-semibold" disabled={!agree} onClick={()=>{ /* placeholder - implement payment flow */ setIsCheckoutModalOpen(false); }}>
            Pay Now
          </button>
        </div>
      </div>
